...
export async function GET(req: NextRequest) {
  try {
    const res = await queryHv5(
      'SELECT id, nome, sigla FROM imob_hv5.apoestado ORDER BY nome'
    );
...
export async function POST(req: NextRequest) {
  try {
    ...
    const checkRes = await queryHv5(
      `SELECT id FROM imob_hv5.apoestado WHERE TRIM(UPPER(sigla)) = $1 OR translate(TRIM(UPPER(sigla)), 'ÁÀÂÃÄÉÈÊËÍÌÎÏÓÒÔÕÖÚÙÛÜÇ', 'AAAAAEEEEIIIIOOOOOUUUUC') = $1`,
      [cleanSigla]
    );
