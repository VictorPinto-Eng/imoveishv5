  ...
  try {
    const res = await query(`
      SELECT 
        ...
        (SELECT COUNT(*)::int FROM public.produto_servico p WHERE p.imbempreendimento_id = e.id AND p.imbtipoanuncio_id = 1 AND p.statusimovel IN (1, 2)) AS total_unidades,
