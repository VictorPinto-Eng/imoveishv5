...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const citiesRes = await queryHv5(
      'SELECT id, descricao as nome FROM hv5.apocidade WHERE estado_id = $1 ORDER BY descricao',
      [estadoId]
    );
...
export async function POST(req: NextRequest) {
  try {
    ...
    const checkRes = await queryHv5(
      `SELECT id FROM hv5.apocidade WHERE estado_id = $1 AND (TRIM(UPPER(descricao)) = $2 OR translate(TRIM(UPPER(descricao)), 'ÁÀÂÃÄÉÈÊËÍÌÎÏÓÒÔÕÖÚÙÛÜÇ', 'AAAAAEEEEIIIIOOOOOUUUUC') = $2)`,
      [estado_id, cleanDesc]
    );
