  ...
  try {
    ...
    if (!cityId && uf && cidade) {
      const stateRes = await queryHv5(
        'SELECT id FROM hv5.apoestado WHERE sigla = $1',
        [uf.toUpperCase()]
      );
      ...
      const cityRes = await queryHv5(
        'SELECT id FROM hv5.apocidade WHERE UPPER(descricao) = $1 AND estado_id = $2',
    ...
    const bairrosRes = await queryHv5(
      'SELECT id, descricao as nome FROM hv5.apobairro WHERE cidade_id = $1 ORDER BY descricao',
      [cityId]
    );
...
export async function POST(req: NextRequest) {
  try {
    ...
    const checkRes = await queryHv5(
      `SELECT id FROM hv5.apobairro WHERE cidade_id = $1 AND (TRIM(UPPER(descricao)) = $2 OR translate(TRIM(UPPER(descricao)), 'ÁÀÂÃÄÉÈÊËÍÌÎÏÓÒÔÕÖÚÙÛÜÇ', 'AAAAAEEEEIIIIOOOOOUUUUC') = $2)`,
      [cidade_id, cleanDesc]
    );
    ...
    const insertRes = await queryHv5(
      'INSERT INTO hv5.apobairro (descricao, cidade_id, estado_id) VALUES ($1, $2, $3) RETURNING id',
