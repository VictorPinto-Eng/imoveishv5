  ...
  try {
    ...
    const res = await query(`
      SELECT 
        ...
        (SELECT MIN(COALESCE(NULLIF(c.area_util, 0), NULLIF(c.area_construida, 0), NULLIF(c.area_terreno, 0))) FROM public.produto_servico ps LEFT JOIN public.produto_servico_carac c ON ps.id = c.produto_servico_id WHERE ps.imbempreendimento_id = p.imbempreendimento_id AND ps.statusimovel IN (1, 2)) AS emp_min_area,
