...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const res = await query(`
      SELECT 
        p.*,
        (SELECT COUNT(*)::int FROM public.produto_servico ps WHERE ps.imbempreendimento_id = p.imbempreendimento_id AND ps.ativo = true AND ps.imbtipoanuncio_id = 1) AS emp_total_unidades,
        (SELECT MIN(COALESCE(pl.preco_base, pv.preco_base, 0)) FROM public.produto_servico ps LEFT JOIN public.produto_servicos_loca pl ON ps.id = pl.produto_servico_id LEFT JOIN public.produto_servicos_venda pv ON ps.id = pv.produto_servico_id WHERE ps.imbempreendimento_id = p.imbempreendimento_id AND ps.ativo = true AND ps.imbtipoanuncio_id = 1) AS emp_min_preco,
        (SELECT MIN(c.area_util) FROM public.produto_servico ps LEFT JOIN public.produto_servico_carac c ON ps.id = c.produto_servico_id WHERE ps.imbempreendimento_id = p.imbempreendimento_id AND ps.ativo = true AND ps.imbtipoanuncio_id = 1) AS emp_min_area,
