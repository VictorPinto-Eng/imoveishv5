...
export async function generateMetadata({ params }: { params: Promise<{ slug: string[] }> }): Promise<Metadata> {
    ...
    const rawImage = imovel.foto_capa || imovel.imagens_urls?.[0] || '/logo_hv5_1024.png'
    const imageUrl = rawImage.startsWith('http') ? rawImage : `${baseUrl}${rawImage.startsWith('/') ? '' : '/'}${rawImage}`
    const ogImage = `${baseUrl}/_next/image?url=${encodeURIComponent(imageUrl)}&w=1200&q=80`
    ...
    return {
        ...
        openGraph: {
            ...
            images: ogImage ? [{
                ...
                width: 1200,
                height: 630,
                type: ogImage.endsWith('.png') ? 'image/png' : (ogImage.endsWith('.jpg') || ogImage.endsWith('.jpeg') ? 'image/jpeg' : 'image/webp'),
                alt: title,
            }] : [],
            url: canonicalUrl,
            type: 'website',
